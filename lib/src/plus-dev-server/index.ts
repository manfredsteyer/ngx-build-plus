// 
//  Fork of WebpackDevServerConfigurationOptions
//  due to lack of an extension point
//  also see: https://github.com/angular/angular-cli/issues/11281
//
...
export class DevServerBuilder implements Builder<DevServerBuilderOptions> {
  ...
  extendWebpackConfig(config: any, options: DevServerBuilderOptions, projectRoot: Path): any {
  ...
  run(builderConfig: BuilderConfiguration<DevServerBuilderOptions>): Observable<BuildEvent> {
    ...
    return checkPort(options.port, options.host).pipe(
      ...
      concatMap(() => new Observable(obs => {
        ...
        let webpackConfig = browserBuilder.buildWebpackConfig(
          root, projectRoot, host, browserOptions as NormalizedBrowserBuilderSchema);
        
        webpackConfig = this.extendWebpackConfig(webpackConfig, options, projectRoot);
